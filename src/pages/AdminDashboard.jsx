...
export default function AdminDashboard({ session, profile }) {
  ...
  // Statystyki - frekwencja per dzień tygodnia
  const dayStats = [0, 1, 2, 3, 4, 5, 6].map(day => {
    const dayNames = ["Nd", "Pn", "Wt", "Śr", "Cz", "Pt", "So"];
    const dayBookings = allBookings.filter(b => new Date(b.classes?.starts_at).getDay() === day);
    return { name: dayNames[day], count: dayBookings.length };
  });
  ...
  allBookings.forEach(b => {
    const h = new Date(b.classes?.starts_at).getHours();
  ...
  const notifIcon = (type) => {
    ...
    if (type === "waitlist_promoted") return "⬆️";
    return "🔔";
  };
  ...
  return (
    <div className="app-layout">
      ...
      <main className="main-content">
        ...
        {tab === "notifications" && (
          <>
            <div className="page-header"><h2>Powiadomienia</h2><p>Aktywność klientów w czasie rzeczywistym</p></div>
        ...
        {tab === "stats" && (
          <>
            <div className="page-header"><h2>Statystyki</h2><p>Przegląd aktywności studia</p></div>
            <div className="stats-row">
              ...
              <div style={{ display: "flex", alignItems: "flex-end", gap: "0.75rem", height: 160 }}>
                {dayStats.map(d => (
                  <div key={d.name} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: "0.4rem" }}>
                    ...
                    <div style={{ width: "100%", background: "var(--sage)", borderRadius: "4px 4px 0 0", height: `${(d.count / maxDay) * 120}px`, minHeight: d.count > 0 ? 8 : 0, transition: "height 0.3s" }} />
            ...
            <div className="card">
              {topHours.length === 0 ? (
                <p style={{ color: "var(--mid)", fontSize: "0.9rem" }}>Brak danych</p>
        ...
        {tab === "participants" && selectedClass && (
          <>
            ...
            {participants.length === 0 ? (
              ...
              <div className="table-wrapper">
                <table>
                  <thead><tr><th>#</th><th>Imię i nazwisko</th><th>Email</th><th>Data zapisu</th><th>Akcja</th></tr></thead>
                  <tbody>
        ...
        {tab === "history" && (
          <>
            ...
            {pastClasses.length === 0 ? (
              <div className="empty-state"><div className="empty-icon">📋</div><p>Brak minionych zajęć</p></div>
            ) : (
              <div className="table-wrapper">
                <table>
        ...
        {tab === "clients" && (
          <>
            ...
            {allProfiles.length === 0 ? (
              <div className="empty-state"><div className="empty-icon">👥</div><p>Brak klientów</p></div>
            ) : (
              <div className="table-wrapper">
                <table>
                  <thead><tr><th>Imię i nazwisko</th><th>Email</th><th>Rezerwacje</th><th>Ostatnie zajęcia</th></tr></thead>
                  <tbody>
                    {allProfiles.map((c, i) => {
                      const clientBookings = allBookings.filter(b => b.user_id === c.id);
                      const sorted = [...clientBookings].sort((a, b) => new Date(b.classes?.starts_at || 0) - new Date(a.classes?.starts_at || 0));
                      return (
                        <tr key={i}>
                          <td><strong>{c.first_name} {c.last_name}</strong></td>
                          <td>{c.email}</td>
                          <td>{clientBookings.length}</td>
                          <td>{sorted[0]?.classes?.starts_at ? formatDate(sorted[0].classes.starts_at) : "—"}</td>
                        </tr>
                      );
                    })}
                  </tbody>
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowModal(false)}>
          <div className="modal">
            ...
            <div className="form-group">
              ...
              <input className="form-input" placeholder="np. Pilates Flow" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />
            </div>
            <div className="form-group">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
              <div className="form-group">
                ...
                <input className="form-input" type="number" min="15" max="180" step="15" value={form.duration_min} onChange={e => setForm({ ...form, duration_min: +e.target.value })} />
              </div>
              <div className="form-group">
                <label className="form-label">Maks. miejsc</label>
                <input className="form-input" type="number" min="1" max="50" value={form.max_spots} onChange={e => setForm({ ...form, max_spots: +e.target.value })} />
              </div>
            </div>
            <div className="form-group">
              <label className="form-label">Lokalizacja (opcjonalnie)</label>
              <input className="form-input" placeholder="np. Sala A" value={form.location} onChange={e => setForm({ ...form, location: e.target.value })} />
            </div>
            <div className="form-group">
              <label className="form-label">Notatki (opcjonalnie)</label>
              <input className="form-input" placeholder="np. Przynieś matę" value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} />
            </div>
